...
export const getTweetsForAllUsers = async (idOfCurrentUser: string) => {
...
export const getTweetsByText = async (idOfCurrentUser: string, textTweet: string) => {
  const tweetComponents: ITweetComponent[] = [];
  const tweetsCollectionRef = collection(db, COLLECTIONS.TWEETS);

  const searchQueryText: keyof ITweet = 'text';

  const queryTweets = query(
    tweetsCollectionRef,
    where(searchQueryText, '>=', textTweet),
    where(searchQueryText, '<', `${textTweet}\uf8ff`),
  );
